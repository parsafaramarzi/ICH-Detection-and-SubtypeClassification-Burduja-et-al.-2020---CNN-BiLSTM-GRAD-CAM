...
## ⚠️ Data Licensing and Compliance (IMPORTANT)
...
```
/data
├── hemorrhage_diagnosis_raw_ct.csv   # Master CSV with slice-level labels
└── ct_scans/
    ├── 001.nii                      # NIfTI file for Patient 1 (3D Volume)
    ├── 002.nii
    └── ...
```
...
The `src/train.py` script executes the entire training pipeline. The `HemorrhageDataset` wrapper class ensures that a fixed number of slices (defaulting to 16) is sampled from each patient's 3D volume before batching, which is critical for preventing crashes due to variable sequence lengths.
